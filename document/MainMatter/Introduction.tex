...
A raiz de estos y otros requerimientos funcionales, se han modificado numerosas redes existentes de anta\~no y se han creado otras espec\'ificas al sector empresarial. En este \'utltimo grupo se enmarca Hyperledger Fabric [\cite{androulaki2018hyperledger}], como una tecnolog\'ia de redes distribu\'idas (del ingl\'es \emph{Distributed Ledger Technology DLT}) dise\~nada para encajar en este panorama.%****Aqui es valido hacer la aclaracion de que DLT no es precisamente blockchain, sino q es mas amplio
%destacar sobresalir dominar

La naturaleza permisionada de HF permite que los participantes de la red se conozcan entre s\'i, lo cual le aporta un gran nivel de seguridad. 
